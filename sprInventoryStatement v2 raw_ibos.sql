...
DECLARE	@BusinessUnitId bigint=0, @WhId bigint=NULL, @PlantId bigint=0, @ItemTypeId int=null,@CategoryId int=null,@SubCategoryId int=null, @FromDate Datetime='2023-07-01',@ToDate DateTime=getdate(), @PageNo int, @PageSize int, @Search nvarchar(300)=null 
...
BEGIN
	...
	SET @dteOpenToDate=DATEADD(dd, 1, @ToDate)
	 SELECT @TotalRows= count( DISTINCT i.intItemId)  
	 ...
	 SELECT row_number() OVER (ORDER BY t.ItemId) as Sl,*
	 	
		INTO #TempALL
	 FROM(
		SELECT    t.strWhName,t.intWhId, t.ItemId, t.strItemName, t.strCode as strItemCode, pw.strBaseUOM AS strUomName, 
		(select STUFF((select '; '+'['+''+'(BIN: '+strBinNumber+')]' from wms.tblItemPlantWarehouse pl 
		inner join wms.tblInventoryLocation l on pl.IntInventoryLocationId=l.intInventoryLocationId
		where pl.IntInventoryLocationId!=@damageId and pl.intItemId=t.ItemId  group by strBinNumber FOR XML PATH('')), 1, 1, '') ) AS strLocation_BIN,
		...
		SELECT Sl , @TotalRows TotalRows, t.strWhName,  
		t.ItemId, t.strItemName,  strItemCode,   strUomName,  strLocation_BIN,
		isnull(numOpenQty,0) numOpenQty,isnull(numOpenValue,0) numOpenValue,numProductionQty,numProductionValue,numReceiveQty,
		numReceiveValue,numIssueQty,numIssueValue,numAdjustQty,numAdjustValue,numPurReturnQty,
		numPurReturnValue,numTransOutQty,numTransOutValue,numTransInQty,numTransInValue,numOtherQty,numOtherValue,
